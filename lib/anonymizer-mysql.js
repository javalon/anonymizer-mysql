...
module.exports = {
    ...
    return new Promise(function(resolve, reject) {
      ...
      mysql.createConnection(thisSelf.config.target_db).then(function(connection) {
        var queries = thisSelf.config.rules.map(function(query_data) {
          var action = query_data.insertIgnore ? "INSERT IGNORE " : "UPDATE ";
          var query = action + query_data.table + " SET ";
          query_data.fields.forEach(function(field) {
            query += field.field + " = " + field.apply + ", ";
          });
          var whereQuery =
            query_data.where && query_data.where.length > 0 ? " where " + query_data.where : "";
  ...
  migrationDatabaseProcess: function() {
    ...
    return new Promise(function(resolve, reject) {
      ...
      var dumpv8 = thisSelf.commander.dumpv8 || false;
      ...
      if (dumpExists && mysqlExists) {
        ...
        if (thisSelf.config.target_db.user && thisSelf.config.target_db.password) {
          pipe =
            ...
            execMysql +
            " -P " +
            thisSelf.config.target_db.port +
            " -h " +
            thisSelf.config.target_db.host +
            " -u " +
        ...
        cmd.get(dump + pipe, function(error, data, stderr) {
        ...
        var message = !dumpExists
          ? "mysqldump is necessary. Please check the options below.\n"
          : "";
  ...
  createTargetDatabase: function() {
    ...
    return new Promise(function(resolve, reject) {
      ...
      thisSelf.target_connection.query(
        "CREATE DATABASE " + thisSelf.config.target_db.database,
        function(error) {
          if (error) {
            reject({
